...
from PyQt5.QtWidgets import QApplication, QLabel, QMainWindow, QLineEdit, QPushButton, QComboBox, QRadioButton
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.adress_line.resize(500, 40)
        self.adress_line.move(20, 640)
        self.adress_line.setFocusPolicy(Qt.NoFocus)
        self.adress_line.setReadOnly(True)


        self.button = QPushButton("Искать", self)
        self.button.resize(80, 40)
        self.button.move(240, 580)
        self.button.clicked.connect(self.update)
        self.button.setFocusPolicy(Qt.NoFocus)

        self.clear_button = QPushButton("Сбросить местоположение", self)
        self.clear_button.resize(200, 40)
        self.clear_button.move(590, 640)
        self.clear_button.clicked.connect(self.remove_cur_place)
        self.clear_button.setFocusPolicy(Qt.NoFocus)

        self.radio = QRadioButton(self)
        self.radio.setFocusPolicy(Qt.ClickFocus)
        self.radio.move(590, 530)
    ...
    def update(self):
            ...
            if str(self.line.text()) != "":
                ...
                try:
                    if self.radio.isChecked():
                        ...
                        postal_code = ""
                    self.adress_line.setText(full_adress + postal_code)
                except:
                    self.adress_line.setText(full_adress)
                self.lon = coords[0]
                self.lat = coords[1]
                self.pt_list.append(coords)
                self.line.setText("")
        except:
            pass
        lon = str(self.lon)
        ...
        try:
            params = {
                ...
                "l": self.typeMap,
                "z": self.z,
                "pt": "~".join([",".join(coords) for coords in self.pt_list]),
                "size": ",".join(["650", "450"])
            }
        except:
            params = {
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_CapsLock or event.key() == Qt.Key_Shift:
            return
    ...
    def remove_cur_place(self):
        params = {
            "ll": ",".join([str(37.622513), str(55.75322)]),
            "l": "map",
            "z": self.z,
            "pt": "~".join([",".join(coords) for coords in self.pt_list]),
            "size": ",".join(["650", "450"])
